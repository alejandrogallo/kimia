...
#+begin_src lisp :noweb-ref kimia :eval no
...
(defequiv :c++ integer
  ...
  :caster-body (lambda (ty)
                    (format nil "return (size_t)new (~a)(ecl_to_int(o));"
                            (translate :c++ ty))))
...
(defequiv :c++ (const F)
  ...
  :caster-header (lambda (ty) (caster-header :c++ (cadr ty)))
...
(assert-equal (caster-signature :c++ '(struct (uttu integer)))
              "size_t s_uttu_of_clint (const cl_object o);")
...
#+RESULTS:
: (:TRANSLATE #<bytecompiled-function 0x24c99b0> :DECLARE-VAR
:  #<bytecompiled-function 0x24c9b90> :DEFINE #<bytecompiled-function 0x24c9c30>
:  :CASTER-HEADER #<bytecompiled-function 0x24c9a00> :CASTER-NAME
:  #<bytecompiled-function 0x24c9a50> :CASTER-BODY
:  #<bytecompiled-function 0x24c9af0> :CASTER-SNIPPET
:  #<bytecompiled-closure #<bytecompiled-function 0x3951a00>>)
