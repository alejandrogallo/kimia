...
(defmacro defequiv (lang type
  ...
  (flet ((fun-or-scalar (thing)
    ...
    (let* ((type-lang-name (defequiv-var-name lang type))
           ...
           (caster-snippet-f (fun-or-scalar caster-snippet))
               ...
               '(:translate ,translate
                 ...
                 :caster-snippet ,(lambda (ty)
                                    (format
                                     ...
                                     (indent 2 (funcall caster-snippet-f ty)))))))
      )))
...
(defequiv :c++ (pointer F)
  ...
  :caster-name (lambda (ty)
                 (format nil "p~a" (caster-name :c++ (cadr ty)))))
...
(defequiv :c++ (const F)
  ...
  :caster-snippet (lambda (ty) (caster-snippet :c++ (cadr ty))))
#+end_src
