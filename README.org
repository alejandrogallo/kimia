...
(deftype-c++
  ...
  :declare-var (lambda (type type-name var-name)
             ...
             (assert (eq (length type) 2))
             (format nil
                     "vector<~a> ~a;"
                     (translation-c++ (cadr type))
                     var-name)
             )
  :define "")
#+end_src
...
(defmacro c++-declare-var-fn-default (translate)
  `(lambda (ty vn)
     (format nil "~a ~a;" (funcall ,translate ty) vn)))
...
(deftype-c++ double-float
  :translate "double")

(deftype-c++ single-float
  :translate "float")
...
#+RESULTS:
: (:TRANSLATE #<bytecompiled-function 0x1b76370> :DECLARE-VAR
:  #<bytecompiled-closure #<bytecompiled-function 0x1b76a00>> :DEFINE
:  #<bytecompiled-function 0x1b762d0>)
...
(defun declare-var-struct-c++ (ty vn)
  (let* ((name (cadr ty))
    ...
    (format nil "~a ~a;" pre-var vn)))


(defun translate-struct-c++ (ty)
  (let* ((ty-name (cadr ty))
         (name (if ty-name (string-capitalize ty-name) ""))
         (fields (caddr ty)))
    (concatenate 'string
                 "struct " (or name "")
                 " {" (endl)
...
#+RESULTS:
: (:TRANSLATE #<bytecompiled-function TRANSLATE-ENUM-C++ 0x1b76230> :DECLARE-VAR
:  #<bytecompiled-function DECLARE-VAR-ENUM-C++ 0x1b76b90> :DEFINE
:  #<bytecompiled-function DEFINE-ENUM-C++ 0x2473690>)
...
struct Tensor-Reader {
  struct  {
    std::string FILE;
    std::string MODE;
  } IN;
  struct  {
    std::string TENSOR-PATH;
    struct  {
      int ORDER;
      std::vector< int > LENS;
    } LENS;
