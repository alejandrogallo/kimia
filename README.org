...
is simply just
#+begin_src lisp :noweb-ref vector-cpp-caster-snippet :eval no
(lambda (ty)
  (format nil
          +c++-vector-snippet+
          (translate :c++ ty)
...
#+begin_src lisp :eval no :noweb-ref test-kimia
(assert-equal (translate :c++ '(vector double-float))
              "std::vector< double >")
(assert-equal (translate :c++ '(vector (g 5)))
              "std::vector< _G5 >")
(assert-equal (translate :c++ '(vector (g 5) 8))
              "std::array< _G5, 8 >")
(assert-equal (translate :c++ '(vector (vector (vector double-float) 2) 8))
              "std::array< std::array< std::vector< double >, 2 >, 8 >")

;;; CASTING
(assert-equal (caster-name :c++ '(vector integer 8))
              "ar_of_8_clint")

(assert-equal (caster-snippet :c++ '(vector integer))
...
(assert-equal (caster-name :c++ '(vector (vector double-float) 8))
              "ar_of_8_v_of_cldouble")

(assert-equal (caster-snippet :c++ '(vector (vector double-float) 8))
              "
...
(eval-when (:compile-toplevel :load-toplevel :execute)
  ...
  (defun struct-check-type (cons-struct ty)
    ...
    (let* ((spec (struct-get-expanded-spec ty))
      ...
      (dolist (key (get-keys cons-struct))
        (let* ((type (find key fields :test
                           (lambda (k x) (getf x k)))))
          (print key)
          (print (typep (getf cons-struct key) (getf type key)))
          ))
      ))
...
(defgenericstruct
    ...
    ((:vectorspace A)
     (:fields (vector B))
     (:lens (vector tensor-field))
     (:mask-tensor (vector tensor-field))
...
;; struct-typep
;(struct-typep '(:name "hello world" :lens #(5d0 9d0 8d0))
...
(defmacro assert-equal (one two)
  `(let ((left-hand ,one)
         (right-hand ,two))
     (assert (equal left-hand right-hand))))
